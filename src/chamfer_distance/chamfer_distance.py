...
nn_distance_module = tf.load_op_library(os.path.join(module_path, 'core', 'tf_nndistance_so.so'))
...
class ChamferDistance:
    def __init__(self, verbose=False):
        ...
        with self.graph.as_default():
            self.inp1 = tf.compat.v1.placeholder(shape=[None, None, 3], dtype=tf.float32)
            self.inp2 = tf.compat.v1.placeholder(shape=[None, None, 3], dtype=tf.float32)
            self.dist1, self.idx1, self.dist2, self.idx2 = nn_distance(self.inp1, self.inp2)
            self.dist1_avg = tf.reduce_mean(input_tensor=self.dist1, axis=1)
            self.dist2_avg = tf.reduce_mean(input_tensor=self.dist2, axis=1)
...
def _demo():
    import random
    cd_api = ChamferDistance(verbose=True)
    ...
    print(chamfer_distance.evaluate(xyz1, xyz2))
